var barrageWall = {
    ...
    upWall: function(img, user, txt) { //弹幕上墙
        ...
        if (txt.length >= 40) {
            elem = $('<div ></div>') //创建元素
                .addClass('list') //添加样式
                .css("top", this.lastElem * 140 + "px") //定位
                .html(`<img src='${img}' alt=''/> ${user}: <div class="barrage-box">${txt}</div>`) //添加内容
                .appendTo(this.container); //追加在父元素后面
        } else {
            elem = $('<div style="font-size:16px;" ></div>') //创建元素
                .addClass('list') //添加样式
                .css("top", this.lastElem * 40 + "px") //定位
                .html(`<img src='${img}' alt=''/> ${user}: <div>${txt}</div>`) //添加内容
                .appendTo(this.container); //追加在父元素后面
        }
