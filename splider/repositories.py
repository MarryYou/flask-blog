...
def getGithub():
    ...
    proxies = {"http": "10.167.219.228:8118", "https": "10.167.219.228:8118"}
    res = requests.get(
        url="https://github.com/MarryYou?tab=repositories",
        headers=headers,
    )
    if res.status_code == 200:
        ...
        html = (
            ...
            .find("div", attrs={"id": "user-repositories-list"})
            .find("ul")
        )
        openSources = []
        for child in html.contents:
            if child.name != None:
                ...
                desc = None if child.find("p") == None else child.find("p").string
                svg = child.find(
